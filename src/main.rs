...
fn main() {
    let foo = Expression::BinaryOperatorExpression {
        lhs: &Expression::ValueExpression(123),
        operator: Operator::Add,
        rhs: &Expression::PrefixOperatorExpression {
            operator: Operator::Add,
            expression: &Expression::VariableExpression("test".to_string()),
        },
    };
    println!("{:?}", foo);
}
